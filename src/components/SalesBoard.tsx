...
export function SalesBoard({ contacts, broadcasts }: { contacts: Contact[]; broadcasts: Broadcast[] }) {
  ...
  }, [contacts, search, type, market, status, tag, emailOnly, sort]);

  // broadcast target: selected contacts if any, else the filtered segment
  ...
  const segmentLabel = selected.size > 0
    ...
    : [type && TYPE_LABEL[type], market, status && STATUS_LABEL[status], tag && `#${tag}`, emailOnly && "has-email"].filter(Boolean).join(" · ") || "All contacts";
  ...
  function clearFilters() { setSearch(""); setType(""); setMarket(""); setStatus(""); setTag(""); setEmailOnly(false); }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2 text-sm">
        {[["Total", stats.total], ["Agents", stats.agents], ["Partners", stats.partners], ["Emailable", stats.emailable]].map(([l, n]) => (
      ...
      <div className="flex flex-wrap items-center gap-2 rounded-xl border border-[var(--color-sand)] bg-white/40 p-3">
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search name / brokerage / email" className={`${field} min-w-[200px] flex-1`} />
            ...
            {filtered.map((c) => (
              <tr key={c.id} className={`border-b border-[var(--color-sand)]/50 ${selected.has(c.id) ? "bg-[var(--color-accent)]/10" : ""}`}>
                ...
                <td className="py-3 pr-4 font-medium">
                  {c.fullName}
                  {c.email ? <div className="text-xs text-[var(--color-muted)]">{c.email}</div> : <div className="text-xs text-[var(--color-muted)]/60 italic">no email</div>}
                </td>
                ...
                <td className="py-3 pr-4">
                  <div className="flex flex-wrap gap-1">
                    {(c.tags || []).map((t) => (
                      <button key={t} onClick={() => setTag(t)} className="rounded-full bg-[var(--color-sand)]/60 px-2 py-0.5 text-[0.6rem] hover:bg-[var(--color-accent)]" title={`Filter by ${t}`}>{t}</button>
                    ))}
                  </div>
                </td>
                <td className="py-3 pr-4">
